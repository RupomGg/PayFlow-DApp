...
function ManagerDashboard({ contract, account }) {
    ...
    const loadManagerData = React.useCallback(async () => {
        try {
            ...
            // Get monthly wage and days worked
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
                        ...
                        <div className="group relative bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 overflow-hidden">
                            ...
                            <div className="relative">
                                ...
                                <p className="text-3xl font-extrabold text-gray-900">{managerData.dailyRate}</p>
                        ...
                        <div className="group relative bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden">
                            ...
                            <div className="relative">
                                ...
                                <p className="text-3xl font-extrabold text-white">{managerData.currentUnpaidAmount}</p>
                                <p className="text-sm text-emerald-100 mt-1 font-medium">ETH pending</p>
                            </div>
                        </div>

                        {/* Late Strikes */}
                        <div className={`group relative rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden ${managerData.lateStrikes > 0 ? 'bg-gradient-to-br from-red-500 to-orange-600' : 'bg-white border border-gray-100'}`}>
                            <div className={`absolute inset-0 ${managerData.lateStrikes > 0 ? 'bg-white/10' : 'bg-gradient-to-br from-gray-50/50 to-transparent'} opacity-0 group-hover:opacity-100 transition-opacity duration-500`}></div>
                            <div className="relative">
                                <div className={`flex items-center gap-2 mb-3 ${managerData.lateStrikes > 0 ? 'text-white' : 'text-gray-600'}`}>
                                    <div className={`${managerData.lateStrikes > 0 ? 'bg-white/20 backdrop-blur-sm' : 'bg-gray-100'} p-2 rounded-lg`}>
                                        <AlertTriangle className="h-5 w-5" />
                                    </div>
                                    <span className="text-sm font-semibold">Late Strikes</span>
                                </div>
                                <p className={`text-3xl font-extrabold ${managerData.lateStrikes > 0 ? 'text-white' : 'text-gray-900'}`}>{managerData.lateStrikes} <span className="text-xl opacity-70">/ 4</span></p>
                                <p className={`text-sm mt-1 font-medium ${managerData.lateStrikes > 0 ? 'text-red-100' : 'text-gray-500'}`}>{managerData.lateStrikes > 0 ? 'Be careful!' : 'Perfect record'}</p>
                            </div>
                        </div>
                    ...
                    <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl p-6 border border-gray-200">
                        <div className="flex items-center gap-3 text-gray-600">
                            ...
                            <p className="font-medium">
                                Last Check-in: <span className="text-gray-900 font-bold">{managerData.lastCheckIn == 0 ? 'Never' : new Date(managerData.lastCheckIn * 1000).toLocaleString()}</span>
                            </p>
                        </div>
                    </div>
                                ...
                                <div key={idx} className="group relative bg-gradient-to-br from-gray-50 to-gray-100 p-5 rounded-2xl border border-gray-200 hover:border-emerald-300 hover:shadow-lg transition-all duration-300">
                                    ...
                                    <div className="relative">
                                        ...
                                        <button 
                                            ...
                                            className="flex items-center justify-center gap-2 w-full bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-white py-2.5 rounded-xl text-sm font-semibold transition-all shadow-sm hover:shadow-md active:scale-[0.98] transform"
                                        >
