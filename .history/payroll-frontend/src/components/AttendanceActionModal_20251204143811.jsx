...
const AttendanceActionModal = ({ isOpen, onClose, contract, employeeAddress, actionType: initialActionType, onSuccess }) => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [success, setSuccess] = useState(false);
  const [selectedDates, setSelectedDates] = useState([]);
  const [actionType, setActionType] = useState(initialActionType || null);
            ...
            <>
              ...
              <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg flex gap-3">
                ...
                <div className="text-sm text-blue-900">
                  Select the type of attendance correction you want to perform for this employee.
                </div>
              </div>
              ...
              <div className="space-y-4">
                {/* Reset Late Button */}
                <button
                  onClick={() => setActionType('resetLate')}
                  className="w-full p-6 bg-gradient-to-br from-yellow-50 to-yellow-100 hover:from-yellow-100 hover:to-yellow-200 border-2 border-yellow-300 rounded-xl transition-all group text-left"
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-lg font-bold text-gray-900 mb-1">Reset Late Attendance</h3>
                      <p className="text-sm text-gray-700">
                        Convert <strong className="text-yellow-700">Late</strong> attendance records to{' '}
                        <strong className="text-green-700">Present</strong>. Select date range with late records.
                      </p>
                    </div>
                  </div>
                </button>
                ...
                {/* Fix Absent Button */}
                <button
                  onClick={() => setActionType('fixAbsent')}
                  className="w-full p-6 bg-gradient-to-br from-red-50 to-red-100 hover:from-red-100 hover:to-red-200 border-2 border-red-300 rounded-xl transition-all group text-left"
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-lg font-bold text-gray-900 mb-1">Fix Absent to Present</h3>
                      <p className="text-sm text-gray-700">
                        Convert <strong className="text-red-700">Absent</strong> attendance records to{' '}
                        <strong className="text-green-700">Present</strong>. This will increase days worked and salary.
                      </p>
                    </div>
                  </div>
                </button>
                ...
                <div className="text-sm text-blue-900">
                  ...
                  {actionType === 'fixAbsent' && (
                    <>
                      ...
                      <strong>Present</strong>. This will increase days worked and the employee's pending salary. Other statuses will be ignored.
                    </>
                  )}
                </div>
              </div>
