            ...
            <div className="w-full bg-black min-h-[calc(100vh-80px)]">
                <div className="w-full px-6 py-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                        <div className="bg-gradient-to-br from-purple-900/40 to-purple-900/20 border border-purple-500/30 rounded-xl p-6 hover:border-purple-500/60 transition-all">
                            ...
                            <p className="text-4xl font-bold text-purple-300">{managerData.dailyRate}</p>
                            <p className="text-xs text-gray-500 mt-2">ETH per day</p>
                        </div>

                        <div className="bg-gradient-to-br from-blue-900/40 to-blue-900/20 border border-blue-500/30 rounded-xl p-6 hover:border-blue-500/60 transition-all">
                            <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="bg-gradient-to-br from-emerald-900/40 to-emerald-900/20 border border-emerald-500/30 rounded-xl p-6 hover:border-emerald-500/60 transition-all">
                            <div className="flex items-start justify-between mb-4">
                            ...
                            <p className="text-4xl font-bold text-emerald-300">{managerData.currentUnpaidAmount}</p>
                            <p className="text-xs text-gray-500 mt-2">ETH pending</p>
                        </div>

                        <div className={`bg-gradient-to-br ${managerData.lateStrikes > 0 ? 'from-red-900/40 to-orange-900/20 border-red-500/30' : 'from-gray-800 to-gray-900 border-gray-700'} border rounded-xl p-6 hover:border-opacity-60 transition-all`}>
                            <div className="flex items-start justify-between mb-4">
                                <div className={`${managerData.lateStrikes > 0 ? 'bg-red-500/20' : 'bg-gray-700'} p-3 rounded-lg`}>
                                    <AlertTriangle className={`h-6 w-6 ${managerData.lateStrikes > 0 ? 'text-red-400' : 'text-gray-400'}`} />
                                </div>
                            </div>
                            <p className="text-gray-400 text-sm font-medium mb-1">Late Strikes</p>
                            <p className={`text-4xl font-bold ${managerData.lateStrikes > 0 ? 'text-red-300' : 'text-gray-300'}`}>{managerData.lateStrikes}/4</p>
                            <p className="text-xs text-gray-500 mt-2">{managerData.lateStrikes > 0 ? 'Be careful!' : 'Perfect'}</p>
                        </div>

                        <div className="bg-gradient-to-br from-indigo-900/40 to-indigo-900/20 border border-indigo-500/30 rounded-xl p-6 hover:border-indigo-500/60 transition-all">
                            <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 rounded-xl p-6 hover:border-emerald-500/50 transition-all">
                            <button onClick={() => toggleSection('onboard')} className="w-full flex items-center justify-between mb-4 hover:opacity-80">
                                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                                    <UserPlus className="h-5 w-5 text-emerald-400" />
                                    Onboard Employee
                                </h3>
                                <ChevronDown className={`h-5 w-5 text-gray-400 transition-transform ${expandedSections.onboard ? 'rotate-180' : ''}`} />
                            </button>
                            {expandedSections.onboard && (
                                <div className="space-y-3">
                                    <input value={newEmpAddress} placeholder="0x..." onChange={e => setNewEmpAddress(e.target.value)} className="w-full px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-400 outline-none text-sm font-mono text-gray-200" />
                                    <input value={newEmpName} placeholder="Full Name" onChange={e => setNewEmpName(e.target.value)} className="w-full px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-400 outline-none text-sm text-gray-200" />
                                    <input value={newEmpPhone} placeholder="Phone" onChange={e => setNewEmpPhone(e.target.value)} className="w-full px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-400 outline-none text-sm text-gray-200" />
                                    <input value={newEmpRate} placeholder="Daily Rate (ETH)" onChange={e => setNewEmpRate(e.target.value)} className="w-full px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-400 outline-none text-sm text-gray-200" />
                        ...
                        <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 rounded-xl p-6 hover:border-amber-500/50 transition-all">
                            <button onClick={() => toggleSection('shift')} className="w-full flex items-center justify-between mb-4 hover:opacity-80">
                                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                                    <Clock className="h-5 w-5 text-amber-400" />
                                    Assign Shift
                                </h3>
                                <ChevronDown className={`h-5 w-5 text-gray-400 transition-transform ${expandedSections.shift ? 'rotate-180' : ''}`} />
                            </button>
                            {expandedSections.shift && (
                                <div className="space-y-3">
                                    <input value={shiftEmp} placeholder="Employee Address" onChange={e => setShiftEmp(e.target.value)} className="w-full px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg focus:ring-2 focus:ring-amber-400 outline-none text-sm font-mono text-gray-200" />
                                    <select value={shiftTime} onChange={e => setShiftTime(e.target.value)} className="w-full px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg focus:ring-2 focus:ring-amber-400 outline-none text-sm text-gray-200">
                                        <option value="32400">🌅 9:00 AM</option>
                                        <option value="50400">☀️ 2:00 PM</option>
                                        <option value="79200">🌙 10:00 PM</option>
                                    </select>
                                    <button onClick={assignShift} className="w-full bg-amber-600 hover:bg-amber-700 text-white font-semibold py-2 rounded-lg text-sm transition-all">Assign</button>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 rounded-xl p-6 hover:border-blue-500/50 transition-all">
                            <button onClick={() => toggleSection('rate')} className="w-full flex items-center justify-between mb-4 hover:opacity-80">
                                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                                    <TrendingUp className="h-5 w-5 text-blue-400" />
                                    Rate Change
                                </h3>
                                <ChevronDown className={`h-5 w-5 text-gray-400 transition-transform ${expandedSections.rate ? 'rotate-180' : ''}`} />
                            </button>
                            {expandedSections.rate && (
                                <div className="space-y-3">
                                    <input value={rateChangeEmp} placeholder="Employee Address" onChange={e => setRateChangeEmp(e.target.value)} className="w-full px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-400 outline-none text-sm font-mono text-gray-200" />
                                    <input value={newRate} placeholder="New Rate (ETH)" onChange={e => setNewRate(e.target.value)} className="w-full px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-400 outline-none text-sm text-gray-200" />
                                    <button onClick={requestRateChange} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 rounded-lg text-sm transition-all">Request</button>
                                </div>
                            )}
                        </div>
                    </div>
                                ...
                                <div className="grid grid-cols-1 gap-3 max-h-96 overflow-y-auto">
                                    {myEmployees.map((emp, idx) => (
                                        <div key={idx} className="bg-gray-700/50 p-4 rounded-lg border border-gray-600">
                                            <p className="font-mono text-xs text-gray-400 truncate mb-2">{emp}</p>
                                            <button 
                                                onClick={() => { setSelectedEmployeeForAttendance(emp); setAttendanceModalOpen(true); }} 
                                                className="w-full bg-amber-600 hover:bg-amber-700 text-white py-2 rounded-lg text-sm font-semibold transition-all"
                                            >
                            ...
                            {payoutHistory.length === 0 ? (
                                ...
                                <div className="space-y-3 max-h-96 overflow-y-auto">
                                    {payoutHistory.map((record, idx) => (
                                        <div key={idx} className="flex justify-between items-center p-4 bg-gray-700/50 rounded-lg border border-gray-600">
                                            <div>
                                                <p className="font-bold text-emerald-300">{Web3.utils.fromWei(record.amount, 'ether')} ETH</p>
                                                <p className="text-xs text-gray-400 mt-1">{new Date(record.timestamp * 1000).toLocaleDateString()}</p>
                                            </div>
                                            <p className="text-xs font-semibold text-gray-300 bg-gray-600 px-3 py-1 rounded-full">{record.daysWorked} days</p>
                                        </div>
                                    ))}
                                </div>
                            )}
                    ...
                    <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 rounded-xl p-6">
                        <div className="flex items-center gap-3 text-gray-400">
                            ...
                            <p className="font-medium">Last Check-in: <span className="text-white font-bold">{managerData.lastCheckIn == 0 ? 'Never' : new Date(managerData.lastCheckIn * 1000).toLocaleString()}</span></p>
                        </div>
                    </div>
                </div>
            </div>
