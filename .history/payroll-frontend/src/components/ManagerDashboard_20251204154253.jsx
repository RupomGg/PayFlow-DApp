                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        ...
                        <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 rounded-lg p-3">
                            <div className="mb-2 pb-2 border-b border-gray-700">
                                <div className="flex items-center gap-1.5 text-gray-400">
                                    ...
                                    <p className="text-[10px]">Last: <span className="text-white font-bold">{managerData.lastCheckIn == 0 ? 'Never' : new Date(managerData.lastCheckIn * 1000).toLocaleString()}</span></p>
                                </div>
                            </div>
                        ...
                        <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 rounded-lg p-3">
                            ...
                            {payoutHistory.length === 0 ? (
                                <div className="text-center py-3">
                                    <p className="text-gray-400 text-[10px]">No payouts</p>
                                </div>
                            ) : (
                                <div className="space-y-1.5 max-h-80 overflow-y-auto">
                                    {payoutHistory.map((record, idx) => (
                                        <div key={idx} className="flex justify-between items-center p-1.5 bg-gray-700/50 rounded border border-gray-600">
                                            <div>
                                                <p className="font-bold text-emerald-300 text-xs">{Web3.utils.fromWei(record.amount, 'ether')} ETH</p>
                                                <p className="text-[9px] text-gray-400">{new Date(record.timestamp * 1000).toLocaleDateString()}</p>
                                            </div>
                                            <p className="text-[9px] font-semibold text-gray-300 bg-gray-600 px-1.5 py-0.5 rounded">{record.daysWorked}d</p>
                                        </div>
                                    ))}
                                </div>
                            )}
                        ...
                        <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 rounded-lg p-3">
                            ...
                            <input
                                ...
                                className="w-full px-2 py-1 mb-2 bg-gray-700/50 border border-gray-600 rounded text-[10px] font-mono text-gray-200 focus:ring-1 focus:ring-amber-400 outline-none"
                            />
                            
                            {myEmployees.length === 0 ? (
                                <div className="text-center py-3">
                                    <p className="text-gray-400 text-[10px]">No members</p>
                                </div>
                            ) : (
                                <div className="space-y-1.5 max-h-72 overflow-y-auto">
                                    {myEmployees
                                        ...
                                        <div key={idx} className="bg-gray-700/50 p-2 rounded border border-gray-600">
                                            <div className="mb-1.5">
                                                <p className="text-[10px] font-bold text-amber-300">{employeeDetails[emp]?.name || 'Loading...'}</p>
                                                <p className="font-mono text-[9px] text-gray-400 truncate">{emp}</p>
                                                <p className="text-[9px] text-gray-400">📞 {employeeDetails[emp]?.phone || 'N/A'}</p>
                                                <p className="text-[9px] text-emerald-400">💰 {employeeDetails[emp]?.dailyRate || '0'} ETH/day</p>
                                            </div>
                                            <button 
                                                onClick={() => { setSelectedEmployeeForAttendance(emp); setAttendanceModalOpen(true); }} 
                                                className="w-full bg-amber-600 hover:bg-amber-700 text-white py-0.5 rounded text-[9px] font-semibold transition-all"
                                            >
                                    ...
                                    {myEmployees.filter(emp => emp.toLowerCase().includes(searchQuery.toLowerCase())).length === 0 && (
                                        <div className="text-center py-3">
                                            <p className="text-gray-400 text-[10px]">No results</p>
                                        </div>
                                    )}
                                </div>
                            )}
