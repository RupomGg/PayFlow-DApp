...
function ManagerDashboard({ contract, account }) {
    ...
    const loadMyEmployees = React.useCallback(async () => {
        try {
            ...
            // Load details for each employee
            const details = {};
            for (const empAddr of emps) {
                try {
                    ...
                    console.error(`Error loading details for ${empAddr}:`, err);
                    details[empAddr] = { name: 'Error', phone: 'N/A', dailyRate: '0' };
                }
            }
            ...
            <div className="w-full bg-black min-h-[calc(100vh-80px)]">
                <div className="w-full px-6 py-8 space-y-6">
                    ...
                    {expandedSections.payout && (
                        <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-blue-500/50 rounded-lg p-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                                    {payoutHistory.map((record, idx) => (
                                        <div key={idx} className="bg-gray-700/50 p-3 rounded-lg border border-gray-600">
                                            <p className="font-bold text-emerald-300 text-lg">{Web3.utils.fromWei(record.amount, 'ether')} ETH</p>
                                            <p className="text-xs text-gray-400 mt-1">{new Date(record.timestamp * 1000).toLocaleDateString()}</p>
                                            <p className="text-xs font-semibold text-gray-300 bg-gray-600 px-2 py-0.5 rounded mt-2 inline-block">{record.daysWorked} days</p>
                                        </div>
                                    ))}
                                </div>
                            )}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 rounded-lg p-3">
                            <div className="mb-2 pb-2 border-b border-gray-700">
                                <div className="flex items-center gap-1.5 text-gray-400">
                                    ...
                                    <p className="text-xs">Last: <span className="text-white font-bold">{managerData.lastCheckIn == 0 ? 'Never' : new Date(managerData.lastCheckIn * 1000).toLocaleString()}</span></p>
                                </div>
                            </div>
                        ...
                        <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 rounded-lg p-4">
                            ...
                            <input
                                ...
                                className="w-full px-3 py-2 mb-3 bg-gray-700/50 border border-gray-600 rounded text-xs font-mono text-gray-200 focus:ring-2 focus:ring-amber-400 outline-none"
                            />
                            ...
                            {myEmployees.length === 0 ? (
                                ...
                                <div className="space-y-2 max-h-80 overflow-y-auto pr-2">
                                    {myEmployees
                                        ...
                                        .map((emp, idx) => (
                                        <div key={idx} className="bg-gray-700/50 p-3 rounded-lg border border-gray-600 hover:border-amber-500/50 transition-all">
                                            <div className="mb-2">
                                                <p className="text-sm font-bold text-amber-300">{employeeDetails[emp]?.name || 'Loading...'}</p>
                                                <p className="font-mono text-xs text-gray-400 truncate mt-1">{emp}</p>
                                                <p className="text-xs text-gray-400 mt-1">📞 {employeeDetails[emp]?.phone || 'N/A'}</p>
                                                <p className="text-xs text-emerald-400 mt-1">💰 {employeeDetails[emp]?.dailyRate || '0'} ETH/day</p>
                                            </div>
                                            <button 
                                                onClick={() => { setSelectedEmployeeForAttendance(emp); setAttendanceModalOpen(true); }} 
                                                className="w-full bg-amber-600 hover:bg-amber-700 text-white py-1.5 rounded text-xs font-semibold transition-all flex items-center justify-center gap-1"
                                            >
                                    ...
                                    {myEmployees.filter(emp => emp.toLowerCase().includes(searchQuery.toLowerCase())).length === 0 && (
                                        <div className="text-center py-6">
