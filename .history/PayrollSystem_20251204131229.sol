...
contract PayrollSystem {
    ...
    // Helper function to count present days since a specific date
    function _countPresentDaysSinceDate(address _emp, uint32 _startDate) internal view returns (uint32 count) {
        ...
        for (uint i = 0; i < records.length; i++) {
            if (records[i].date >= _startDate && records[i].status == AttendanceStatus.Present) {
                count++;
            }
        }
    }
    ...
    function _processSalaryPayment(address _emp) internal {
        ...
        uint32 daysWorked = emp.daysWorkedSinceLastPay;
