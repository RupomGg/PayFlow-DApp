...
contract PayrollSystem {
    ...
    struct RateHistory {
        uint256 rate;        // Daily rate in Wei
        uint32 effectiveDate; // Date this rate became effective
    }
    ...
    mapping(address => RateHistory[]) public rateHistory; // Employee -> List of rate changes
    mapping(address => uint32) public lastPaymentDate; // Employee -> Last payment date
    ...
    function addEmployee(address _emp, string calldata _name, string calldata _phone, uint256 _rate) external onlyAdminOrManager {
        ...
        // Initialize rate history with the initial rate
        uint32 today = uint32(block.timestamp / SECONDS_PER_DAY);
        rateHistory[_emp].push(RateHistory({
            rate: _rate,
            effectiveDate: today
        }));
        lastPaymentDate[_emp] = today;
    ...
    function approveRateChange(address _emp) external onlyAdmin {
        ...
        // Add to rate history - new rate effective immediately
        uint32 today = uint32(block.timestamp / SECONDS_PER_DAY);
        rateHistory[_emp].push(RateHistory({
            rate: newRate,
            effectiveDate: today
    ...
    // Helper function to get the effective rate for a specific date
    function _getEffectiveRate(address _emp, uint32 _date) internal view returns (uint256) {
        RateHistory[] storage history = rateHistory[_emp];
        
        // Find the rate effective on this date (most recent rate before or on the date)
        uint256 effectiveRate = 0;
        for (uint i = 0; i < history.length; i++) {
            if (history[i].effectiveDate <= _date) {
                effectiveRate = history[i].rate;
            } else {
                break;
            }
        }
    ...
    // Helper function to calculate wages with historical rates
    function _calculateWagesWithHistoricalRates(address _emp, uint32 _startDate) internal view returns (uint256 totalWages) {
        AttendanceRecord[] storage records = attendanceRecords[_emp];
        
        for (uint i = 0; i < records.length; i++) {
            if (records[i].date >= _startDate && 
                (records[i].status == AttendanceStatus.Present || records[i].status == AttendanceStatus.Late)) {
                uint256 rateForDay = _getEffectiveRate(_emp, records[i].date);
                unchecked {
                    totalWages += rateForDay;
                }
            }
        }
    }
    ...
    function _processSalaryPayment(address _emp) internal {
        ...
        // Use last payment date instead of fixed 30-day lookback
        uint32 today = uint32(block.timestamp / SECONDS_PER_DAY);
        uint32 paymentStartDate = lastPaymentDate[_emp];
        
        // Calculate wages using historical rates
        uint256 wages = _calculateWagesWithHistoricalRates(_emp, paymentStartDate);
        
        if (wages == 0) revert NoUnpaidAmount();
        ...
        // Count days for payout record
        uint32 presentDays = 0;
        AttendanceRecord[] storage records = attendanceRecords[_emp];
        for (uint i = 0; i < records.length; i++) {
            if (records[i].date >= paymentStartDate && 
                (records[i].status == AttendanceStatus.Present || records[i].status == AttendanceStatus.Late)) {
                presentDays++;
            }
        }
    ...
    function getMonthlyWage(address _emp) external view returns (uint256) {
        return _calculateWagesWithHistoricalRates(_emp, lastPaymentDate[_emp]);
    }
